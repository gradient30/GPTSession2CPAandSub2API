...
| 现象 | 处理 |
|------|------|
| Cloudflare job 红，`npx` exit code 1 | 展开 `Verify Cloudflare API access` 步骤：whoami 失败多为 Token/Account ID 错误；whoami 成功但 deploy 失败则检查 Pages 项目名、是否已在 CF 控制台用 Git 集成重复绑定 |
| Cloudflare job 红，Authentication error | 检查两个 Secrets 是否正确、Token 需含 **Account - Cloudflare Pages: Edit** 与 **Account - Account Settings: Read** |
