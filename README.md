...
> 本仓库 fork 自 [gtxx3600/GPTSession2CPAandSub2API](https://github.com/gtxx3600/GPTSession2CPAandSub2API)。
> 上游在线演示：[gtxx3600.github.io/GPTSession2CPAandSub2API](https://gtxx3600.github.io/GPTSession2CPAandSub2API/)。
> **本 fork 的构建版本与 SHA256 以本仓库 `npm run build` 产物为准**；处理真实 token 时建议本地打开或下载 Release 附件。
...
亦可使用上游 GitHub Pages（请自行核对页面内版本号与 SHA256，见 [SECURITY.md](./SECURITY.md)）。

## 安全使用（推荐）

- 处理真实 token：**优先本地打开** `docs/index.html`，或下载 [Release](https://github.com/gradient30/GPTSession2CPAandSub2API/releases) 附件
- 使用在线版时：对比页面显示的 **版本 · 日期 · SHA256** 与仓库 `dist/SHA256SUMS` 或 `meta[name=build-sha256-full]`
- 转换完成后**立即清空输入**并关闭标签页
- 不要将 session JSON 发给他人或粘贴到不可信网页
- 详见 [SECURITY.md](./SECURITY.md) 与 [docs/AUDIT_CHECKLIST.md](./docs/AUDIT_CHECKLIST.md)

## 开发与验证

```bash
npm test          # 单元测试
npm run build     # 从 src/ 生成 docs/index.html
npm run verify    # 校验构建产物并跑测试
npm run acceptance  # 全量计划验收 + 生成 docs/ACCEPTANCE_REPORT.md
```

发布流程见 [docs/RELEASE.md](./docs/RELEASE.md)。

部署到 GitHub Pages 与 Cloudflare Pages（推送 `main` 自动同步）见 **[docs/DEPLOY.md](./docs/DEPLOY.md)**。
...
缺少真实 `id_token` 时会构造带 `id_token_synthetic` 标记的占位 JWT；部分下游工具若校验签名可能失败。
...
所有解析和转换都在浏览器本地完成，不上传 token，不写入本地存储（除本会话敏感确认偏好外）。

## 社区

社区讨论（非本工具技术支持入口）：[Discord 频道](https://discord.gg/GFmHY2TZNy)（`https://discord.gg/GFmHY2TZNy`）
